		...
		<div class="col-md-10 main-content" id="mainContent">
            ...
            <div class="container mt-5">
				<div class="card shadow p-4 position-relative">
					...
					<!-- Edit Profile Form -->
					<form id="updateDoctorForm" action="{{ route('superadmin.update') }}" method="POST" enctype="multipart/form-data">

                         @elseif(session('user.role')==='admin') 
                        <form id="updateDoctorForm" action="{{ route('admin.update') }}" method="POST" enctype="multipart/form-data">
                             @elseif(session('user.role')=== 'frontdesk')
                              <form id="updateDoctorForm" action="{{ route('frontdesk.update') }}" method="POST" enctype="multipart/form-data">
                                 @endif  
						@csrf
